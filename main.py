...
def sync_car_source_from_backend(user_id: str = "", u: str = "", p: str = ""):
    ...
    try:
        ...
        # 🚨 【熔斷機制】保護資料
        if len(all_cars) < 100:
            return {"status": "error", "message": f"🚨 數據異常！後台只回傳了 {len(all_cars)} 筆，系統已自動拒絕更新以保護原始資料，請稍後再試。"}
        ...
        return {"status": "error", "message": f"執行錯誤：{str(e)}"}
    finally:
        gc.collect()
...
def handle_text_message(event):
    ...
    if text in ["更新車源", "抓取車源"]:
        ...
        def run_task():
            try:
                # LINE 機器人呼叫時，也將 user_id 傳入進行權限保護驗證

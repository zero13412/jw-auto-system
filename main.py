    ...
    try:
        ...
        if len(raw_data) > 1:
            for row in raw_data[1:]:
                if len(row) > 1:
                    user_code = str(row[1]).strip() # B 欄：帳號
                    if user_code:
                        pwd = ""
                        if len(row) > 2:
                            pwd = str(row[2]).strip() # C 欄：密碼
                        if not pwd:
                            pwd = "123456" # 空白就預設 123456
...
def api_sync_car_source(user_id: str = "", u: str = "", p: str = ""):
    ...
    if not valid_u: return {"status": "need_login", "message": "⚠️ 公司後台密碼已更改，系統自動嘗試了所有備用通行證皆失敗！\n請手動輸入最新的帳號與密碼。"}
